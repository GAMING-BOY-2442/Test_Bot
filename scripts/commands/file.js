...
module.exports.handleReply = ({ api, event, args, handleReply }) => {
  ...
  api.sendMessage("⚡️ Deleted the following files in the commands folder :\n\n"+msg, event.threadID, event.messageID);
}
...
•Example: commands help`;
    ...
    if(files.length == 0) return api.sendMessage(`⚡️ There are no files in the cache that begin with: ${word}`, event.threadID ,event. messageID);
  ...
  //trong tên có ký tự.....
    ...
    files.forEach(file => {
     ...
     api.sendMessage(`${key}\n\n`+msg+`\n» Reply message by number to delete file. You can reply multiple number separated by space - ✅`, event.threadID, (e, info) => global.client.handleReply.push({
